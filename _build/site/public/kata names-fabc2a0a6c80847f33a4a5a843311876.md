...
## Position - Name
Japanese ordinal numbers such as `Ipponme` and `Nihonme` are often prepended to the name or used to refer to kata or kihon within a set. You can find a set to practice these on their own (`Kyuhonme` -> `Nineth`) [Here]('General flashcards/ordinal_numbers.md').
